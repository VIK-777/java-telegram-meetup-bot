...
public class MeetupCalendarBot extends AbilityBot implements SpringLongPollingBot {
    ...
    return Ability
        ...
        .action(ctx -> {
          ...
          var users = usersRepository.findAll().stream()
              ...
              .toList();
          var usersList = IntStream.range(0, users.size())
              .mapToObj(i -> "%d: %s".formatted(i + 1, users.get(i)))
              .toList();
          ListUtils.partition(usersList, 50).forEach(partition ->
              actionsExecutor.sendMessage(chatId, String.join("\n", partition))
          );
        })
        .build();
  }
  ...
  public Ability upcomingEvents() {
    return Ability
        ...
        .action(ctx -> {
          ...
          actionsExecutor.sendMessage(chatId, messageAndButtons.messageText(), getInlineGrid(messageAndButtons.buttons(), 1), ParseMode.HTML);
        })
        .build();
  }
  ...
  public Ability settings() {
    return Ability
        ...
        .action(ctx -> actionsExecutor.sendMessage(ctx.chatId(), "Notification settings:\n" + NOTIFICATIONS_SETTINGS_TEXT,
            getInlineGrid(getNotificationSettingsButtonsForUser(ctx.chatId(), true), 1)))
        .build();
  }
  ...
  private List<ButtonInfo> getNotificationSettingsButtonsForUser(Long userId, boolean isFromSettings) {
      ...
      switch (callbackQueryAction) {
        case ENABLE_NEW_EVENT_NOTIFICATIONS, DISABLE_NEW_EVENT_NOTIFICATIONS -> {
          ...
          actionsExecutor.updateMessageText(upd, "Thank you! Let's setup additional notifications.\n" + NOTIFICATIONS_SETTINGS_TEXT,
              getInlineGrid(getNotificationSettingsButtonsForUser(chatId, false), 1));
        }
        ...
        case OPEN_UPCOMING_EVENTS -> {
          ...
          actionsExecutor.updateMessageText(upd, messageInfo.messageText(), getInlineGrid(messageInfo.buttons(), 1), ParseMode.HTML);
        }
        default -> {
          if (callbackQueryAction.startsWith("TOGGLE")) {
            ...
            actionsExecutor.updateMessageText(upd, getInlineGrid(getNotificationSettingsButtonsForUser(userId, includeNewEventNotificationsFlag), 1));
          }
        }
      }
    };
  ...
  private Pair<Consumer<Boolean>, Supplier<Boolean>> getGetterAndSetter(UserNotificationToggle callbackAction, User user) {
    return switch (callbackAction) {
      case TOGGLE_NEW_EVENTS_NOTIFICATION, TOGGLE_NEW_EVENTS_NOTIFICATION_FROM_SETTINGS -> Pair.of(user::setSendNotifications, user::getSendNotifications);
      case TOGGLE_INFO_NOTIFICATION, TOGGLE_INFO_NOTIFICATION_FROM_SETTINGS -> Pair.of(user::setSendInfoNotifications, user::getSendInfoNotifications);
      case TOGGLE_1_WEEK_NOTIFICATION, TOGGLE_1_WEEK_NOTIFICATION_FROM_SETTINGS -> Pair.of(user::setOneWeekNotification, user::getOneWeekNotification);
      case TOGGLE_1_DAY_NOTIFICATION, TOGGLE_1_DAY_NOTIFICATION_FROM_SETTINGS -> Pair.of(user::setOneDayNotification, user::getOneDayNotification);
      case TOGGLE_12_HOURS_NOTIFICATION, TOGGLE_12_HOURS_NOTIFICATION_FROM_SETTINGS -> Pair.of(user::setTwelveHoursNotification, user::getTwelveHoursNotification);
      case TOGGLE_6_HOURS_NOTIFICATION, TOGGLE_6_HOURS_NOTIFICATION_FROM_SETTINGS -> Pair.of(user::setSixHoursNotification, user::getSixHoursNotification);
      case TOGGLE_1_HOUR_NOTIFICATION, TOGGLE_1_HOUR_NOTIFICATION_FROM_SETTINGS -> Pair.of(user::setOneHourNotification, user::getOneHourNotification);
    };
  }
